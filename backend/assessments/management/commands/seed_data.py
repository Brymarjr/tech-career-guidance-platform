...
from assessments.models import Question, CareerPath, Milestone
from users.models import LearningResource
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            paths_data = [
                {'code': 'R', 'title': 'The Infrastructure Path', 'weeks': 10},
                {'code': 'I', 'title': 'The Data & AI Path', 'weeks': 12},
                {'code': 'A', 'title': 'The Creative Tech Path', 'weeks': 8},
                {'code': 'S', 'title': 'The Tech Leadership Path', 'weeks': 6},
                {'code': 'E', 'title': 'The Tech Entrepreneur Path', 'weeks': 8},
                {'code': 'C', 'title': 'The Systems & Quality Path', 'weeks': 6},
            ]
            ...
            for p in paths_data:
                obj, _ = CareerPath.objects.update_or_create(
                    code=p['code'],
                    defaults={'title': p['title'], 'duration_weeks': p['weeks']}
                )
                path_objs[p['title']] = obj

            # 2. YOUR EXACT 30 QUESTIONS (Restored from your raw list)
            questions_data = [
