...
function InboxInner() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (socketRef.current?.readyState === WebSocket.OPEN) {
        ...
        setSending(true);
        try {
          const res = await api.post(`users/threads/${activeThread.id}/messages/`, { content: newMessage });
          setMessages([...messages, res.data]);
          setNewMessage("");
          fetchThreads(); 
        } catch (err: any) {
          if (err.response?.status === 403) toast.error("Mentorship has ended.");
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fetchTasks();
    } catch (err) { toast.error("Error assigning task."); }
  };
    ...
    <div className="flex h-screen bg-[#F8FAFC] dark:bg-[#0F172A] overflow-hidden transition-colors duration-500">
      <aside className={`w-full md:w-96 bg-white dark:bg-[#1E293B] border-r border-gray-100 dark:border-slate-800 flex flex-col ${activeThread ? 'hidden md:flex' : 'flex'}`}>
        <div className="p-8 border-b border-gray-50 dark:border-slate-800 space-y-4">
          <button onClick={() => router.push('/dashboard')} className="flex items-center gap-2 text-gray-400 hover:text-[#3730A3] dark:hover:text-white transition-colors font-bold text-sm">
            <ChevronLeft size={18} /> Back to Dashboard
          </button>
          <h1 className="text-3xl font-black text-[#1F2937] dark:text-white tracking-tight">Messages</h1>
        </div>
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {threads.length === 0 ? <div className="text-center p-10 text-gray-400 font-medium">No active conversations.</div> : threads.map((t) => (
            <div key={t.id} onClick={() => setActiveThread(t)} className={`p-5 rounded-3xl cursor-pointer transition-all border-2 ${activeThread?.id === t.id ? 'bg-indigo-50 dark:bg-indigo-900/30 border-indigo-200 dark:border-indigo-500' : 'bg-transparent border-transparent hover:bg-gray-50 dark:hover:bg-slate-800'}`}>
              ...
              <div className="flex justify-between items-center mt-2">
                ...
                {t.unread_count > 0 && <span className="w-5 h-5 bg-[#3730A3] text-white text-[10px] flex items-center justify-center rounded-full font-black shadow-lg animate-bounce">{t.unread_count}</span>}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
            ...
            <>
              <header className="p-6 bg-white dark:bg-[#1E293B] border-b border-gray-100 dark:border-slate-800 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    {activeThread.other_user.is_online ? (
                        ...
                        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Last seen {activeThread.other_user.last_seen ? new Date(activeThread.other_user.last_seen).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'recently'}</span>
                    )}
                    </div>
                </div>
                <button onClick={() => setShowTasks(!showTasks)} className={`flex items-center gap-2 px-4 py-2 rounded-2xl font-bold text-xs transition-all ${showTasks ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'bg-indigo-50 text-indigo-600 dark:bg-slate-800'}`}><ListChecks size={18} /> {showTasks ? 'Hide Tasks' : 'View Path Tasks'}</button>
              </header>
                ...
                {messages.map((m) => {
                  ...
                  return (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={m.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[70%] p-5 rounded-[2rem] shadow-sm ${isMe ? 'bg-[#3730A3] text-white rounded-br-none' : 'bg-white dark:bg-[#1E293B] dark:text-gray-200 border border-gray-100 dark:border-slate-800 rounded-bl-none'}`}>
                        ...
                        <div className={`flex items-center gap-2 mt-2 text-[10px] ${isMe ? 'text-indigo-200' : 'text-gray-400'}`}>
                          ...
                          {isMe && <CheckCheck size={14} className={m.is_read ? "text-sky-400 drop-shadow-[0_0_2px_rgba(56,189,248,0.8)]" : "opacity-40"} />}
                        </div>
                      </div>
                    </motion.div>
                  );
              ...
              <form onSubmit={handleSendMessage} className="p-8 bg-white dark:bg-[#1E293B] border-t border-gray-100 dark:border-slate-800">
                <div className="flex gap-4 items-center bg-gray-50 dark:bg-slate-900 p-2 rounded-[2.5rem] border-2 border-transparent focus-within:border-indigo-500 transition-all">
                  <input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Type your message..." className="flex-1 bg-transparent p-4 outline-none dark:text-white font-medium" />
                  <button disabled={sending} className="bg-[#3730A3] text-white p-4 rounded-full shadow-lg hover:scale-105 transition-all disabled:opacity-50">{sending ? <Loader2 className="animate-spin" /> : <Send size={20} />}</button>
                </div>
              </form>
            </>
          ) : (
            <div className="text-center">
              <div className="w-24 h-24 bg-indigo-50 dark:bg-slate-800 rounded-[2.5rem] flex items-center justify-center text-indigo-500 mx-auto mb-6 shadow-inner"><MessageSquare size={48} /></div>
        ...
        <AnimatePresence>
          {showTasks && activeThread && (
            <motion.aside initial={{ x: 320 }} animate={{ x: 0 }} exit={{ x: 320 }} className="w-80 bg-white dark:bg-[#1E293B] border-l border-gray-100 dark:border-slate-800 flex flex-col h-full shadow-2xl">
                <div className="p-8 border-b border-gray-50 dark:border-slate-800">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-2xl font-black dark:text-white flex items-center gap-2"><Target className="text-indigo-500" /> Tasks</h2>
                        {user?.role === 'MENTOR' && <button onClick={() => setShowTaskForm(!showTaskForm)} className="p-2 bg-indigo-600 text-white rounded-xl shadow-lg hover:rotate-90 transition-all duration-300">{showTaskForm ? <X size={20} /> : <Plus size={20} />}</button>}
                    </div>
                    {showTaskForm && (
                        <motion.form initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} onSubmit={handleCreateTask} className="space-y-3 bg-gray-50 dark:bg-slate-900 p-4 rounded-3xl border border-indigo-100 dark:border-indigo-900/50">
                            <input className="w-full bg-white dark:bg-slate-800 p-3 rounded-2xl text-xs outline-none border border-transparent focus:border-indigo-500 transition-all" placeholder="Task Title" value={taskData.title} onChange={(e) => setTaskData({...taskData, title: e.target.value})} required />
                            <textarea className="w-full bg-white dark:bg-slate-800 p-3 rounded-2xl text-xs outline-none border border-transparent focus:border-indigo-500 transition-all h-24" placeholder="Details..." value={taskData.description} onChange={(e) => setTaskData({...taskData, description: e.target.value})} required />
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4">
                    {tasks.length === 0 ? <div className="text-center py-12"><Clock className="mx-auto text-gray-200 mb-4" size={40} /><p className="text-gray-400 font-bold text-sm">No tasks assigned.</p></div> : tasks.map((task) => (
                        <div key={task.id} className="p-5 bg-white dark:bg-[#0F172A] rounded-[2rem] border-2 border-gray-50 dark:border-slate-800 shadow-sm relative overflow-hidden">
                            <div className="flex justify-between items-center mb-3">
                                <span className={`text-[9px] font-black px-2 py-1 rounded-full uppercase ${task.status === 'APPROVED' ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'}`}>{task.status}</span>
                                <div className="flex items-center gap-1 text-indigo-500 font-black text-[10px]"><Award size={14} /> +{task.xp_reward} XP</div>
                            </div>
                            ...
                            {user?.role === 'STUDENT' && task.status === 'PENDING' && <button onClick={() => handleUpdateTask(task.id, 'COMPLETED')} className="w-full mt-4 bg-indigo-600 text-white py-3 rounded-2xl text-[10px] font-black uppercase">Mark Done</button>}
                            {user?.role === 'MENTOR' && task.status === 'COMPLETED' && <button onClick={() => handleUpdateTask(task.id, 'APPROVED')} className="w-full mt-4 bg-emerald-500 text-white py-3 rounded-2xl text-[10px] font-black uppercase">Approve XP</button>}
                        </div>
                    ))}
                </div>
            </motion.aside>
